...
def render_runway_health(transactions_data):
    ...
    with st.container(border=True):
        ...
        with col3:
            st.metric("Estimasi Bertahan", f"{days_display}")
    
    # Logic peringatan
    if runway_days < 30 and current_balance > 0:
        st.warning(f"Peringatan: Runway Anda tersisa kurang dari sebulan ({days_display}). Segera cairkan piutang atau kurangi pengeluaran.")
    elif current_balance <= 0 and len(transactions_data) > 0:
        st.error("Perhatian: Saldo utama Anda saat ini kosong atau minus.")
