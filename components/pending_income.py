            ...
            if submitted_invoice:
                if client and amount > 0:
                    try:
                        ...
                        st.error(f"❌ Gagal menambahkan tagihan. Silakan coba lagi. ({e})")
                else:
    ...
    for item in pending_items:
        with st.container(border=True):
            ...
            if st.session_state.get(f"show_tax_{item['id']}", False):
                st.markdown("---")
                tax_pct = st.slider(
                    f"Potong Persentase ke Pajak/Biaya Admin (%) untuk {item['client_name']}",
                    min_value=0.0, max_value=30.0, value=0.0, step=1.0,
                    key=f"tax_slider_{item['id']}"
                )
                ...
                with col_c1:
                    if st.button("Konfirmasi Cairkan Saldo", key=f"confirm_{item['id']}", type="primary", use_container_width=True):
                        try:
                            ...
                            st.error(f"❌ Gagal mencairkan faktur. Silakan coba lagi. ({e})")
                with col_c2:
